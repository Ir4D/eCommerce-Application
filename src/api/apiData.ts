export const apiDataAdmin = {
  PROJECT_KEY: 'rss-ecom-app',
  CLIENT_ID: 'tdIsIEbnK71vruL5Z3hwP9Fx',
  CLIENT_SECRET: '8P26HVz-pnxEpRSbBw29n5pMXGwRUfCp',
  AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
  API_URL: 'https://api.europe-west1.gcp.commercetools.com',
  SCOPES: 'manage_project:rss-ecom-app'
};

export const apiData = {
  PROJECT_KEY: 'rss-ecom-app',
  AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
  API_URL: 'https://api.europe-west1.gcp.commercetools.com'
};

export const apiDataCredentials = {
  PROJECT_KEY: 'rss-ecom-app',
  CLIENT_ID: 'LGMaLMw5cF5eP6Dh5o9B12un',
  CLIENT_SECRET: 'Yy5fz_Riz0t9rqDVfWZEAw3GCDnqbZGe',
  AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
  API_URL: 'https://api.europe-west1.gcp.commercetools.com',
  SCOPES:
    'create_anonymous_token:rss-ecom-app view_published_products:rss-ecom-app view_categories:rss-ecom-app'
};

export const apiDataAnonymous = {
  PROJECT_KEY: 'rss-ecom-app',
  CLIENT_ID: 'DvNMoxgsmbuytw_zn-ScLQlq',
  CLIENT_SECRET: 'CSf5cBcJ78B2ewNi2xEtr9owqCmztB_i',
  AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
  API_URL: 'https://api.europe-west1.gcp.commercetools.com',
  SCOPES:
    'create_anonymous_token:rss-ecom-app manage_my_quotes:rss-ecom-app manage_my_quote_requests:rss-ecom-app manage_my_profile:rss-ecom-app manage_my_orders:rss-ecom-app manage_my_shopping_lists:rss-ecom-app view_categories:rss-ecom-app manage_my_business_units:rss-ecom-app manage_my_payments:rss-ecom-app view_published_products:rss-ecom-app'
};

export const apiDataPassword = {
  PROJECT_KEY: 'rss-ecom-app',
  CLIENT_ID: 'jUyy4pZA6YbgVBKX5elYVCwV',
  CLIENT_SECRET: 'sff0FwIgAwx7tlrmtZVtNzNGgbLigLwa',
  AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
  API_URL: 'https://api.europe-west1.gcp.commercetools.com',
  SCOPES:
    'create_anonymous_token:rss-ecom-app manage_my_quotes:rss-ecom-app manage_my_quote_requests:rss-ecom-app manage_my_profile:rss-ecom-app manage_my_orders:rss-ecom-app manage_my_shopping_lists:rss-ecom-app view_categories:rss-ecom-app manage_my_business_units:rss-ecom-app manage_my_payments:rss-ecom-app view_published_products:rss-ecom-app'
};

// export const apiDataAnonymous2 = {
//   PROJECT_KEY: 'rss-ecom-app',
//   CLIENT_ID: 'xrXuXkZevT3vraD8MPFJ7bHG',
//   CLIENT_SECRET: 'HTCiWt_zKoHImoJ-fkusvCf15ii3InoG',
//   AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
//   API_URL: 'https://api.europe-west1.gcp.commercetools.com',
//   SCOPES:
//     'create_anonymous_token:rss-ecom-app manage_my_quotes:rss-ecom-app manage_my_quote_requests:rss-ecom-app manage_my_profile:rss-ecom-app manage_my_orders:rss-ecom-app manage_my_shopping_lists:rss-ecom-app view_categories:rss-ecom-app view_orders:rss-ecom-app manage_my_business_units:rss-ecom-app manage_my_payments:rss-ecom-app view_published_products:rss-ecom-app'
// };

// API anonymous 3 (+ manage_customers, manage_orders)
export const apiDataAnonymous2 = {
  PROJECT_KEY: 'rss-ecom-app',
  CLIENT_ID: '180IGHVCgwm3gnFBCv2DBc81',
  CLIENT_SECRET: 'gyfRVg-Lr5NtP-WxCyMMDd8G7l6h3999',
  AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
  API_URL: 'https://api.europe-west1.gcp.commercetools.com',
  SCOPES:
    'create_anonymous_token:rss-ecom-app manage_customers:rss-ecom-app manage_my_quotes:rss-ecom-app manage_my_quote_requests:rss-ecom-app manage_my_profile:rss-ecom-app manage_my_orders:rss-ecom-app manage_my_shopping_lists:rss-ecom-app view_categories:rss-ecom-app view_orders:rss-ecom-app manage_orders:rss-ecom-app manage_my_business_units:rss-ecom-app manage_my_payments:rss-ecom-app view_published_products:rss-ecom-app'
};

// export const apiDataPassManageCustomers = {
//   PROJECT_KEY: 'rss-ecom-app',
//   CLIENT_ID: 'pAMvorhpN5_-mQDEarU8KpCh',
//   CLIENT_SECRET: '9N5Hf6958dKW2paKAVCL1OvWTnft5_XN',
//   AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
//   API_URL: 'https://api.europe-west1.gcp.commercetools.com',
//   SCOPES:
//     'create_anonymous_token:rss-ecom-app manage_customers:rss-ecom-app manage_my_quotes:rss-ecom-app manage_my_quote_requests:rss-ecom-app manage_my_profile:rss-ecom-app manage_my_orders:rss-ecom-app manage_my_shopping_lists:rss-ecom-app view_categories:rss-ecom-app manage_my_business_units:rss-ecom-app manage_my_payments:rss-ecom-app view_published_products:rss-ecom-app'
// };

// API password + manage customers (copy from 09.09)
export const apiDataPassManageCustomers = {
  PROJECT_KEY: 'rss-ecom-app',
  CLIENT_ID: 'oyFim6eNsHcA6dB9Pt5Anwvl',
  CLIENT_SECRET: 'lynic285T29Y_3Hh_iCIHj4N32f_ZFcJ',
  AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
  API_URL: 'https://api.europe-west1.gcp.commercetools.com',
  SCOPES:
    'create_anonymous_token:rss-ecom-app manage_customers:rss-ecom-app manage_my_quotes:rss-ecom-app manage_my_quote_requests:rss-ecom-app manage_my_profile:rss-ecom-app manage_my_orders:rss-ecom-app manage_my_shopping_lists:rss-ecom-app view_categories:rss-ecom-app manage_orders:rss-ecom-app manage_my_business_units:rss-ecom-app manage_my_payments:rss-ecom-app view_published_products:rss-ecom-app'
};
