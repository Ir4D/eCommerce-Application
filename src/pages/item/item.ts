...
const getContent = (
  ...
  if (!catalogItem.masterVariant.images) throw new Error('error');
  return `
  <section class="item-content">
      <div class="item-img_container">
        <img src="${catalogItem.masterVariant.images[0].url}" class="item-img" width="450" height="450" />
      </div>
      <div class="itsem-description catalog-link">
      <button class="btn catalog-card_btn">${category}</button>
      <h2 class="">${catalogItem.name.en}</h2>
...
export default class ItemView extends State {
  ...
  public async render(catalogItem: ProductProjection): Promise<HTMLElement> {
    // console.log(catalogItem.masterVariant.images[0].url)
    /* State.categories?.body.results.forEach((category) => {
        console.log(category.id, category.name.en)}); */
    // if(!category)throw new Error('error');

    const category: string = getCategory(catalogItem.categories[0].id);
    this.container.innerHTML += getContent(catalogItem, category);

    // console.log(catalogItem.categories);
    // console.log(getCategory(catalogItem.categories[0].id))
